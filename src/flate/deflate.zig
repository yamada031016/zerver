...
pub fn compress(comptime container: Container, reader: anytype, writer: *std.Io.Writer, options: Options) !void {
    ...
    return struct {
        ...
        pub fn compress(self: *Self, reader: anytype) !void {
            while (true) {
                ...
                const n = try @constCast(&reader).allocRemaining(std.heap.page_allocator, .unlimited);
                self.hasher.update(buf[0..n.len]);
                self.win.written(n.len);
                // Process window
                try self.tokenize(.none);
