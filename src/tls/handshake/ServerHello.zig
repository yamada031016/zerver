...
pub const ServerHello = struct {
    ...
    pub fn toBytes(self: *const ServerHello) ![]u8 {
        ...
        sh_pos += 1;
        sh[sh_pos] = @intCast(((2 + content.len + extensions.len) & 0xFF0000) >> 16);
        sh[sh_pos + 1] = @intCast(((2 + content.len + extensions.len) & 0x00FF00) >> 16);
        sh[sh_pos + 2] = @intCast((2 + content.len + extensions.len) & 0x0000FF);
        sh_pos += 3;
